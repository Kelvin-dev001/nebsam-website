...
| **V46** | **No Supabase project and no Docker — the database cannot be applied** | *Raised Sprint 3.* All 10 migrations, the RLS policies and the public views are written and consistency-checked, but nothing has been applied and no types have been generated. Blocks: applying migrations, `supabase gen types` (brief PART 7.2 forbids hand-written database types), seeding, and Supabase Auth for the admin. **Unblocked by either** a Supabase project (URL + anon key + service-role key in `.env.local`) **or** Docker Desktop for a local stack. Everything else in Sprint 3 is complete and reviewable without it | Client | Sprint 4 onwards | OPEN |
