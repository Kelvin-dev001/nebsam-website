...
| V04 | **Certificate data source & format** — CSV import, manual entry, or both? Is the certificate number sequential? | Determines import tooling and whether enumeration hardening is needed on the number as well as the plate. The legacy specimen shows format `P051510924U/2007` and 1-year validity | Client | Sprint 3, Sprint 11 | OPEN |
...
| **V46** | **Database types are still hand-written — `supabase gen types` cannot authenticate** | *Raised Sprint 3; narrowed 2 Sep 2026.* **Largely resolved:** the Supabase project exists, all 10 migrations are applied, and `npm run verify:db` confirms against the live database that all 30 tables are present, 17/17 public views are anon-readable, **no base table is anon-readable and no table accepts an anon write**. Seed reference data landed; every customer-bearing table is empty. **What remains:** `types/database.ts` is still the hand-written placeholder, which brief PART 7.2 forbids. Generation needs a Supabase **personal access token** (`SUPABASE_ACCESS_TOKEN`, from Account → Access Tokens) **or** the database password for `--db-url` — the anon and service-role keys cannot authenticate the generator. Until then every `lib/content/` return type is asserted rather than derived, and schema drift would be silent | Client | Sprint 4 onwards | OPEN — narrowed |
