...
| **V50** | **Three HIGH severity vulnerabilities in `sharp` / libvips** | *Raised Sprint 9.* `npm audit` reports CVE-2026-33327, CVE-2026-33328, CVE-2026-35590 and CVE-2026-35591 against `sharp <0.35.0`, which ships transitively with Next.js and performs image optimisation for `next/image`. Relevant because this site processes uploaded imagery and will accept admin uploads from Sprint 12. **The only offered fix is `npm audit fix --force`, which installs `next@16.3.4` — a breaking major upgrade** across the whole app, and CLAUDE.md §3.5 requires a stack change to be proposed and approved rather than taken mid-sprint. Options: (a) upgrade Next in a dedicated sprint with the migration guide, (b) pin a patched `sharp` via an override if one exists for the current Next, (c) accept the risk while `main` still serves the CRA site and no upload path is live. **Must be resolved before Sprint 12 ships uploads and before Sprint 15 goes to production** | Client + Claude | Sprint 12, Sprint 15 | OPEN |
| **V49** | **Sprint 7 (Commerce) is DEFERRED, not skipped** | *Raised 3 Sep 2026, client decision.* Sprint 7 delivers price and VAT labelling, categories, filters, cart, order creation, WhatsApp handoff and order lookup. It was deferred because the catalogue it would sell is about 40% built: 10 of ~26 products published, four held as drafts under A01-A07, and **V05 (installation terms) unanswered for every category**. A07 in particular is a *bundling and pricing* claim — "comes complete with Hybrid Tracker package (optional)" — which may need a bundle concept in the data model before a cart can price it. Building commerce first would mean a cart around products that cannot be priced. **Blocks nothing that Sprint 8+ needs**, but it blocks revenue, and it must not be forgotten: the shop is one of the three commercial objectives in CLAUDE.md §1. Prerequisites: A01-A07, V05, and enough of the catalogue to be worth a cart | Client + Claude | Revenue; must land before Sprint 15 | DEFERRED |
