...
| ~~V54~~ | **A rebuild, and the CMS publish button, could serve a stale database read** | **FIXED 5 Sep 2026, same day.** *Mechanism proven, not inferred:* the cached entry for `/public_blog_posts?slug=eq.what-is-a-container-e-seal` was decoded out of `.next/cache/fetch-cache` holding the OLD `seo_title` while the database held the new one, with `revalidate: 3600` and **`tags: []`**. Those empty tags were the defect. The CMS already called `revalidatePath`, which correctly discarded the rendered page — the page then re-rendered and read the row straight back out of the data cache, so **the publish button could not publish**. *First fix tried and rejected on measurement:* `cache: no-store` on the Supabase client removed the staleness but moved the homepage, `/solutions`, `/products`, `/industries`, all three resources routes and `/sitemap.xml` from prerendered to server-rendered on demand — trading a staleness bug for a performance regression on every index page. *Shipped fix:* every read is tagged with the PostgREST view it hit (`sb:public_blog_posts`), and the blog actions call `revalidateTag` alongside `revalidatePath`, now also covering the resources hub and the sitemap. **Verified:** static generation intact (265 build artefacts, indexes back to prerendered, 1h revalidate), and **133 of 133** cached reads carry tags across 12 views, none untagged. **Residual gap, accepted and documented:** a change made directly in SQL invalidates nothing and can still be served stale for up to an hour — that is how this was found, and it is a developer action with a developer remedy. **Still to verify:** the publish → revalidateTag → fresh render round trip has not been exercised end to end, because no admin account exists (outstanding from Sprint 9) | Claude | — | **CLOSED — one verification pending an admin account** |
