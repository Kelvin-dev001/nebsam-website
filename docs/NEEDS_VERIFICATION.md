...
| **V54** | **A rebuild does not reliably pick up a database change** — the CMS can publish and the site can still serve the old value | *Raised 5 Sep 2026, Sprint 10.* Migration 0036 corrected three `seo_title` values. A full `npm run build` afterwards produced HTML in `.next/server` still carrying the OLD titles while the database held the new ones — so the build read stale data, rather than an old artefact being served. `.next/cache/fetch-cache` (134 entries) is the probable cause: Supabase reads go through `fetch` and page `revalidate` is 3600. The cache was not cleared to confirm, so the mechanism is inferred rather than proven. **This matters well beyond three titles.** Sprint 9 shipped a CMS so non-technical staff can publish, and the whole point of it is that publishing makes content appear. If a deploy can serve a stale read for up to an hour, the CMS is not doing the job it was built for, and the blog dies quietly — which CLAUDE.md §15 names as a way this project fails. **Not fixed in Sprint 10:** the remedy is a caching change across the whole content layer (a no-store fetch on the Supabase client, or tag-based revalidation on publish), which is a data-layer decision under CLAUDE.md §3.5 rather than something to change mid-sprint. Reproduction is in the Sprint 10 report | Claude + Client | Sprint 12; Sprint 15 | OPEN — **decision required** |
