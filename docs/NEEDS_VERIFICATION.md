...
| **V53** | **Re-accept the performance budget miss — the Sprint 4 acceptance was taken on numbers that were wrong** | *Raised 5 Sep 2026.* On 2 Sep 2026 the client accepted a 378 ms LCP miss and directed Sprint 4 to proceed. The report it was given showed **Performance 95**, **TBT 80 ms** and the reassurance that *observed LCP was 237 ms, identical to FCP — no real late paint*. Every one of those readings was taken while `loading.tsx` suppressed hydration on every route (**ADR-0003**), so no page-level client JavaScript ever ran during any measurement. Measured with hydration working: **Performance 80–82**, **TBT 475–596 ms**, **LCP 2.88–3.09 s**, LCP render delay **2 630 ms**. The acceptance therefore covered a smaller, differently-caused miss than the one that exists, and is withdrawn. **Nine of eleven budgets still hold** — CLS, page weight, per-route JS, accessibility, best practices, SEO and font count are all inside budget, and page weight is at 296–306 KB against 1.0–1.5 MB. Three options, costed, with the trade-off named on each: **ADR-0004**. Evidence: `docs/PERFORMANCE_BASELINE.md` | Client | Sprint 14 gate; brief PART 14 | OPEN — **decision required** |
| **V47** | **LCP and Lighthouse Performance both miss the mobile budget** — LCP 2.88–3.09 s against ≤ 2.5 s, and Performance 80–82 against ≥ 90 | *Raised Sprint 4; re-measured and re-diagnosed 5 Sep 2026.* **The Sprint 4 analysis is superseded.** It recorded observed LCP as identical to FCP and TBT as 80 ms, and concluded the cause was critical-path bytes. Both readings were taken while a `loading.tsx` suppressed hydration on every route — fixed in `3d58ddf` — so no page-level client JS ever executed, on any route, in any measurement before 4 Sep 2026. Re-measured with hydration working, median of three: **Performance 80 / 80 / 82** and **TBT 475–596 ms** on homepage, solution and product. A control run with `/_next/static/chunks/*` blocked scores **97**, with **LCP 2.412 s — inside budget** — so the LCP miss and the Performance miss are one problem, not two, and that problem is client-side JavaScript. 122 KB of it costs **814 ms of script evaluation** on a 4× throttled CPU, most of that the React/Next App Router runtime rather than this project’s four client components. Full numbers, method, control and ranked fix candidates: `docs/PERFORMANCE_BASELINE.md`. **Decision needed:** spend a sprint on the candidates — favicon 191 KB → ~5 KB, font instancing, hydration surface — or revise the ≥ 90 budget, because the available items may not close a 10-point gap. **The Sprint 4 client acceptance is withdrawn** — it covered a 378 ms LCP miss, not a 16-point Performance miss, and it was given on the strength of readings that no longer stand. Re-acceptance is raised separately as **V53**, with the options and trade-offs written up in **ADR-0004** | Client + Claude | Sprint 14 | **OPEN — re-diagnosed 5 Sep 2026; acceptance withdrawn, superseded by V53** |
